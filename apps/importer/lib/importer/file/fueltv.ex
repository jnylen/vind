defmodule Importer.File.FuelTV do
  @moduledoc """
  Importer for FuelTV
  """
  ...
  @fields [
    ...
    %{regex: ~r/^(CEST|CET)/i, type: :start_time},
    %{regex: ~r/^Show$/i, type: :content_title},
  ...
  defp process_item(program, fields, channel) do
    ...
    |> parse_datetime(Text.fetch_map_key(program, fields, :start_time), "CET")
    |> case do
      {:ok, datetime} ->
        %{
          start_time: datetime,
