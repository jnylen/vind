defmodule Importer.File.Matkanalen do
  @moduledoc """
  Importer for Matkanalen
  """
  ...
  @fields [
    ...
    %{regex: ~r/^Tid/i, type: :start_time},
    %{regex: ~r/^(Slutt|Slut)/i, type: :end_time},
    %{regex: ~r/^(Programtittel|Programtitel)/i, type: :content_title},
    ...
    %{regex: ~r/^(L.nk til episode bild|Link til episodebilde)/i, type: :image},
  ...
  def import_excel(channel, file_name) do
    ...
    |> case do
      {:ok, datetime} ->
        %{
          ...
          images: program |> Text.fetch_map_key(fields, :image) |> parse_image_url()
        }
  ...
  defp parse_image_url(nil), do: []

  defp parse_image_url(chars) when is_bitstring(chars) do
    [
      %ImageManager.Image{
        type: "content",
        source: chars |> String.replace("dl=0", "dl=1")
      }
    ]
  end

  defp parse_image_url(_), do: []
