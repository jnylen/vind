defmodule Importer.File.BBCNordic do
    ...
    fields =
      for {text, index} <- Enum.with_index(item) do
        cond do
          ...
          Regex.match?(~r/title/i, text) && !Regex.match?(~r/\(/i, text) ->
          ...
          Regex.match?(~r/^series number/i, text) ->
            [season: index]
          ...
          Regex.match?(~r/^presenter/i, text) ->
            [presenters: index]

          Regex.match?(~r/^production year/i, text) ->
  ...
  defp worksheet_to_tz(nil), do: "Europe/Stockholm"

  defp worksheet_to_tz(worksheet) do
    case worksheet |> String.downcase() do
      "norway" -> "Europe/Stockholm"
      "sweden" -> "Europe/Stockholm"
      "denmark" -> "Europe/Stockholm"
      "finland" -> "EET"
      _ -> "CET"
    end
  end
